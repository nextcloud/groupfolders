OC.L10N.register(
    "groupfolders",
    {
    "You cannot remove your own read permission." : "Ní féidir leat do chead léite féin a bhaint.",
    "Group folders" : "Fillteáin ghrúpa",
    "Team folders" : "Fillteáin foirne",
    "Shared folders managed by admins, accessible to designated groups and teams with configurable permissions and quotas" : "Fillteáin chomhroinnte arna mbainistiú ag riarthóirí, inrochtana ag grúpaí agus foirne ainmnithe le ceadanna agus cuótaí inchumraithe",
    "The Group folders app allows administrators to create and manage shared folders for selected\ngroups and teams within Nextcloud.\n\nAdmins can grant one or more groups or teams access to a folder, configure permissions (such\nas read, write, and sharing rights), and assign storage quotas from the Group folders section (under\nadmin settings). The app also supports advanced permissions and integration with Nextcloud’s\ntrash and versioning systems.\n\nAs of Hub 10 / Nextcloud 31, admins must be members of a team to assign that team to a Team\nfolder." : "Cuireann an aip *Group folders* ar chumas riarthóirí fillteáin chomhroinnte a chruthú agus a bhainistiú do ghrúpaí agus d’fhoirne roghnaithe laistigh de Nextcloud.\n\nIs féidir le riarthóirí rochtain ar fhillteán a thabhairt do ghrúpa amháin nó níos mó (nó d’fhoirne), ceadanna a chumrú (amhail cearta léitheoireachta, scríbhneoireachta agus comhroinnte), agus cuótaí stórála a shannadh ón rannán *Group folders* (faoi shocruithe riaracháin). Tacaíonn an aip freisin le hardcheadanna agus le comhtháthú le córais *Trash* (bruscar) agus le córais leaganaithe Nextcloud.\n\nÓ *Hub 10* / *Nextcloud 31* i leith, ní mór do riarthóirí a bheith ina mbaill d’fhoireann chun an fhoireann sin a shannadh d’fhillteán foirne (*Team folder*).",
    "Unset" : "Díshocraithe",
    "Denied (Inherited permission)" : "Diúltaithe (cead oidhreachta)",
    "Allowed (Inherited permission)" : "Ceadaithe (cead oidhreachta)",
    "Denied" : "Diúltaíodh",
    "Allowed" : "Ceadaithe",
    "Access denied" : "Rochtain diúltaithe",
    "Access allowed" : "Ceadaíodh rochtain",
    "Inherit permission" : "Cead oidhreachta",
    "Deny" : "shéanadh",
    "Allow" : "Ceadaigh",
    "Group" : "Grúpa",
    "Team" : "Foireann",
    "Could not save changes. Please try again." : "Níorbh fhéidir na hathruithe a shábháil. Déan iarracht arís.",
    "Advanced permissions for group folder" : "Ceadanna ardleibhéil d'fhillteán grúpa",
    "Group, team, or user" : "Grúpa, foireann, nó úsáideoir",
    "Read" : "Léigh",
    "Write" : "Scríobh",
    "Create" : "Cruthaigh",
    "Delete" : "Scrios",
    "Share" : "Comhroinn",
    "You" : "tú",
    "Remove access rule" : "Bain an riail rochtana",
    "Inherited permissions" : "Ceadanna oidhreachta",
    "Inherited permission cannot be removed" : "Ní féidir cead oidhreachta a bhaint",
    "Add new rule" : "Cuir riail nua leis",
    "Select a user or team" : "Roghnaigh úsáideoir nó foireann",
    "Failed to update encryption setting" : "Theip ar an socrú criptithe a nuashonrú",
    "Group folders encryption" : "Criptiú fillteán grúpa",
    "Encrypt group folders using server-side encryption. Remember that the data cannot be accessed if the encryption key is lost." : "Déan fillteáin ghrúpa a chriptiú ag baint úsáide as criptiú ar thaobh an fhreastalaí. Cuimhnigh nach mbeifear in ann rochtain a fháil ar na sonraí má chailltear an eochair chriptithe.",
    "Enable group folders encryption" : "Cumasaigh criptiú fillteán grúpa",
    "Disabling group folders encryption is only possible using OCC, please refer to the documentation." : "Ní féidir criptiú fillteán grúpa a dhíchumasú ach trí úsáid a bhaint as OCC; féach an doiciméadú le do thoil.",
    "Group folders encryption cannot be enabled on the server because server-side encryption is disabled." : "Ní féidir criptiú fillteán grúpa a chumasú ar an bhfreastalaí toisc go bhfuil criptiú ar thaobh an fhreastalaí díchumasaithe.",
    "Open group folder" : "Oscail fillteán grúpa",
    "List of group folders." : "Liosta fillteán grúpa.",
    "No group folders yet" : "Níl aon fhillteáin ghrúpa fós",
    "Group folders will show up here" : "Taispeánfar fillteáin ghrúpa anseo",
    "Add group" : "Cuir grúpa leis",
    "Default" : "Réamhshocrú",
    "Unlimited" : "Gan teorainn",
    "Folder could not be created" : "Níorbh fhéidir fillteán a chruthú",
    "Are you sure you want to delete \"{folderName}\" and all files inside? This operation cannot be undone" : "An bhfuil tú cinnte gur mhaith leat \"{folderName}\" agus gach comhad istigh a scriosadh? Ní féidir an oibríocht seo a chealú",
    "Delete \"{folderName}\"?" : "An bhfuil fonn ort \"{folderName}\" a scriosadh?",
    "Folder could not be renamed" : "Níorbh fhéidir an fillteán a athainmniú",
    "Group folder admin delegation" : "Grúpáil tarmligean riaracháin fillteán",
    "Nextcloud allows you to delegate the administration of group folders to non-admin users." : "Ligeann Nextcloud duit riarachán fillteán grúpa a tharmligean chuig úsáideoirí nach riarthóirí iad.",
    "Specify below the groups that will be allowed to manage group folders and use its API/REST." : "Sonraigh thíos na grúpaí a mbeidh cead acu fillteáin ghrúpa a bhainistiú agus a API/REST a úsáid.",
    "They will have access to all group folders." : "Beidh rochtain acu ar gach fillteán grúpa.",
    "Specify below the groups that will be allowed to manage group folders and use its API/REST only." : "Sonraigh thíos na grúpaí a mbeidh cead acu fillteáin ghrúpa a bhainistiú agus a API/REST amháin a úsáid.",
    "They will only have access to group folders for which they have advanced permissions." : "Ní bheidh rochtain acu ach ar fhillteáin ghrúpa a bhfuil ceadanna chun cinn acu ina leith.",
    "Group or team" : "Grúpa nó foireann",
    "Sort by number of groups or teams that have access to this folder" : "Sórtáil de réir líon na ngrúpaí nó na bhfoirne a bhfuil rochtain acu ar an bhfillteán seo",
    "Sort by number of groups that have access to this folder" : "Sórtáil de réir líon na ngrúpaí a bhfuil rochtain acu ar an bhfillteán seo",
    "Team folder" : "Fillteán foirne",
    "Folder name" : "Ainm fillteáin",
    "This team folder belongs to a team and cannot be renamed" : "Baineann an fillteán foirne seo le foireann agus ní féidir a ainm a athrú.",
    "Rename \"{mountPoint}\"" : "Athainmnigh \"{mountPoint}\"",
    "Advanced permissions are managed by the team" : "Déanann an fhoireann bainistíocht ar cheadanna ardleibhéil",
    "Enabled" : "Cumasaithe",
    "Disabled" : "Faoi mhíchumas",
    "Advanced permissions allows setting permissions on a per-file basis but comes with a performance overhead" : "Le ceadanna ardleibhéil, is féidir ceadanna a shocrú ar bhonn comhaid amháin ach tá forchostais feidhmíochta ag baint leis.",
    "Advanced permissions for \"{mountPoint}\"" : "Ceadanna ardleibhéil do \"{mountPoint}\"",
    "This folder belongs to a team and cannot be deleted directly; unlink it from its team first" : "Is le foireann an fillteán seo agus ní féidir é a scriosadh go díreach; dínasc é óna fhoireann ar dtús.",
    "No team folders yet" : "Níl aon fhillteáin foirne ann go fóill",
    "No group folders or team folders yet" : "Níl aon fhillteáin ghrúpa ná fillteáin foirne ann go fóill.",
    "Group folder name" : "Ainm an fhillteáin ghrúpa",
    "Create group folder" : "Cruthaigh fillteán grúpa",
    "Do not grant any advanced permissions by default" : "Ná deonaigh aon cheadanna ardleibhéil de réir réamhshocraithe",
    "All" : "Gach",
    "Quota" : "Cuóta",
    "Advanced Permissions" : "Ceadanna Casta",
    "Actions" : "Gníomhartha",
    "Pagination of group folders and team folders" : "Leathanaigh a chruthú do fhillteáin ghrúpa agus d’fhillteáin foirne",
    "Previous" : "Roimhe Seo",
    "Next" : "Ar aghaidh",
    "Page:" : "Leathanach:",
    "User" : "Úsáideoir",
    "Unknown" : "Anaithnid",
    "Users/groups that can manage" : "Úsáideoirí/grúpaí is féidir a bhainistiú",
    "None" : "aon cheann",
    "Unassign" : "Díshannadh",
    "Assign groups" : "Sannadh grúpaí",
    "Edit assigned groups: {groups}" : "Cuir grúpaí sannta in eagar: {groups}",
    "No other groups or teams available" : "Níl aon ghrúpa nó foirne eile ar fáil",
    "No other groups available" : "Níl aon ghrúpa eile ar fáil",
    "{displayName} (team)" : "{displayName} (foireann)",
    "Add group or team" : "Cuir grúpa nó foireann leis",
    "{size} used" : "{size} in úsáid",
    "Other …" : "Eile …",
    "sorted descending" : "sórtáilte ag dul in ísle",
    "sorted ascending" : "sórtáilte ag dul suas",
    "Admin configured folders shared with everyone in a team" : "Fillteáin cumraithe ag an riarthóir a roinntear le gach duine i bhfoireann",
    "Admin configured folders shared with everyone in a team.\n\nFolders can be configured from *Team folders* in the admin settings.\n\nAfter a folder is created, the admin can give access to the folder to one or more teams, control their write/sharing permissions and assign a quota for the folder.\nAs of Hub 10/Nextcloud 31, the admin needs to be a part of the team to be able to assign it a Team folder." : "Fillteáin chumraithe ag an riarthóir a roinntear le gach duine i bhfoireann.\n\nIs féidir fillteáin a chumrú ó *Fillteáin foirne* sna socruithe riarthóra.\n\nTar éis fillteán a chruthú, is féidir leis an riarthóir rochtain ar an bhfillteán a thabhairt d'fhoireann amháin nó níos mó, a gceadanna scríbhneoireachta/comhroinnte a rialú agus cuóta a shannadh don fhillteán.\nÓ Hub 10/Nextcloud 31, ní mór don riarthóir a bheith mar chuid den fhoireann chun go mbeidh sé in ann fillteán Foirne a shannadh dó.",
    "Advanced permissions for Team folder" : "Ceadanna ardleibhéil don fhillteán Foirne",
    "Select a user or group" : "Roghnaigh úsáideoir nó grúpa",
    "Open Team folder" : "Oscail fillteán Foirne",
    "Permission denied. User does not have sufficient permissions." : "Cead diúltaithe. Níl go leor ceadanna ag an úsáideoir.",
    "Unexpected status from server" : "Stádas gan choinne ón bhfreastalaí",
    "List of Team folders." : "Liosta fillteáin Foirne.",
    "No Team folders yet" : "Níl aon fhillteán Foirne fós",
    "Team folders will show up here" : "Taispeánfar fillteáin foirne anseo",
    "Team folder admin delegation" : "Tarmligean riarthóir fillteán foirne",
    "Nextcloud allows you to delegate the administration of Team folders to non-admin users." : "Ligeann Nextcloud duit riarachán fillteán Foirne a tharmligean chuig úsáideoirí nach riarthóirí iad.",
    "Specify below the groups that will be allowed to manage Team folders and use its API/REST." : "Sonraigh thíos na grúpaí a mbeidh cead acu fillteáin Foirne a bhainistiú agus a API/REST a úsáid.",
    "They will have access to all Team folders." : "Beidh rochtain acu ar gach fillteán Foirne.",
    "Specify below the groups that will be allowed to manage Team folders and use its API/REST only." : "Sonraigh thíos na grúpaí a mbeidh cead acu fillteáin Foirne a bhainistiú agus a API/REST amháin a úsáid.",
    "They will only have access to Team folders for which they have advanced permissions." : "Ní bheidh rochtain acu ach ar fhillteáin Foirne a bhfuil ardcheadanna acu ina leith.",
    "Advanced permissions are only supported with Nextcloud 16 and up" : "Ní thacaítear le ceadanna ardleibhéil ach le Nextcloud 16 agus níos airde",
    "Pagination of team folders" : "Leathanachú fillteán foirne",
    "Team Folders" : "Fillteáin Foirne",
    "Shared folders managed by admins, accessible to designated teams with configurable permissions and quotas" : "Fillteáin chomhroinnte arna mbainistiú ag riarthóirí, inrochtana ag foirne ainmnithe le ceadanna agus cuótaí inchumraithe",
    "Team Folders (formerly \"Group Folders\") allows administrators to create and manage shared folders that are accessible\n\t\t\tto selected teams within Nextcloud.\n\n\t\t\tAdmins can configure folders from the Team Folders section in the admin settings, where they can grant access to one\n\t\t\tor more teams, set custom permissions (such as read, write, and sharing rights), and assign storage quotas to each\n\t\t\tfolder.\n\n\t\t\tAs of Hub 10/Nextcloud 31, admins must be members of a team to assign it a Team Folder. The app supports advanced\n\t\t\tfeatures such as quota management, granular access control, and integration with Nextcloud’s trash and versioning\n\t\t\tsystems." : "Le Fillteáin Foirne (ar a dtugtaí \"Fillteáin Ghrúpa\" roimhe seo) is féidir le riarthóirí fillteáin chomhroinnte a chruthú agus a bhainistiú atá inrochtana\t\t\tag foirne roghnaithe laistigh de Nextcloud.Is féidir le riarthóirí fillteáin a chumrú ón rannán Fillteáin Foirne sna socruithe riaracháin, áit ar féidir leo rochtain a dheonú ar fhoireann amháin\t\t\tnó níos mó, ceadanna saincheaptha a shocrú (amhail cearta léite, scríbhneoireachta agus comhroinnte), agus cuótaí stórála a shannadh do gach\t\t\tfillteán.Ó Hub 10/Nextcloud 31, ní mór do riarthóirí a bheith ina mbaill d'fhoireann chun Fillteán Foirne a shannadh di. Tacaíonn an aip le gnéithe ardleibhéil\t\t\tamhail bainistíocht cuótaí, rialú rochtana mionsonraithe, agus comhtháthú le córais bhruscair agus leaganacha Nextcloud.\t\t\t",
    "Team Folders (formerly \"Group Folders\") allows administrators to create and manage shared\nfolders for selected teams within Nextcloud.\n\nAdmins can grant one or more teams access to a folder, configure permissions (such as read,\nwrite, and sharing rights), and assign storage quotas from the Team Folders section (under\nadmin settings). The app also supports advanced permissions and integration with Nextcloud’s\ntrash and versioning systems.\n\nAs of Hub 10 / Nextcloud 31, admins must be members of a team to assign that team to a Team\nFolder." : "Le Fillteáin Foirne (ar a dtugtaí \"Fillteáin Ghrúpa\" roimhe seo) is féidir le riarthóirí fillteáin chomhroinnte\na chruthú agus a bhainistiú do fhoirne roghnaithe laistigh de Nextcloud.\n\nIs féidir le riarthóirí rochtain a dheonú do fhoireann amháin nó níos mó ar fhillteán,\nceadanna a chumrú (amhail cearta léite,\nscríbhneoireachta agus comhroinnte), agus cuótaí stórála a shannadh ón rannán Fillteáin Foirne (faoi\nshocruithe riarthóra). Tacaíonn an aip freisin le ceadanna ardleibhéil agus comhtháthú le córais bhruscair\nagus leaganacha Nextcloud.\n\nÓ Hub 10 / Nextcloud 31, ní mór do riarthóirí a bheith ina mbaill d'fhoireann chun an fhoireann sin a shannadh do\nFhillteán Foirne."
},
"nplurals=5; plural=(n==1 ? 0 : n==2 ? 1 : n<7 ? 2 : n<11 ? 3 : 4);");
