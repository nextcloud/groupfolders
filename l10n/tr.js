OC.L10N.register(
    "groupfolders",
    {
    "You cannot remove your own read permission." : "Kendi okuma izninizi kaldıramazsınız.",
    "Group folders" : "Grup klasörleri",
    "Team folders" : "Takım klasörleri",
    "Shared folders managed by admins, accessible to designated groups and teams with configurable permissions and quotas" : "Yöneticiler tarafından yönetilen, yapılandırılabilir izinleri ve kotaları olan belirli gruplar ve takımlar tarafından erişilebilen paylaşılan klasörler",
    "The Group folders app allows administrators to create and manage shared folders for selected\ngroups and teams within Nextcloud.\n\nAdmins can grant one or more groups or teams access to a folder, configure permissions (such\nas read, write, and sharing rights), and assign storage quotas from the Group folders section (under\nadmin settings). The app also supports advanced permissions and integration with Nextcloud’s\ntrash and versioning systems.\n\nAs of Hub 10 / Nextcloud 31, admins must be members of a team to assign that team to a Team\nfolder." : "Grup Klasörleri uygulaması, yöneticilerin Nextcloud üzerinde seçilmiş gruplar ve takımlar\niçin paylaşılan klasörler oluşturmasını ve yönetmesini sağlar.\n\nYöneticiler, bir veya birkaç gruba veya takıma bir klasöre erişme izni verebilir, izinleri (okuma, \nyazma ve paylaşma gibi) yapılandırabilir ve Grup Klasörleri bölümünden (yönetici\nayarları altından) depolama alanı kotaları atayabilir. Uygulama ayrıca gelişmiş izinleri \ndestekler ve Nextcloud içindeki çöp ve sürüm sistemleri ile bütünleşik çalışır.\n\nHub 10 / Nextcloud 31 sürümüyle, yöneticiler bir takımı bir Takım Klasörüne atayabilmesi için \no takımın üyesi olması gereklidir.",
    "Unset" : "Ayarı kaldır",
    "Denied (Inherited permission)" : "Reddedildi (devralınan izin)",
    "Allowed (Inherited permission)" : "İzin verildi (devralınan izin)",
    "Denied" : "Reddedildi",
    "Allowed" : "İzin verildi",
    "Access denied" : "Erişim reddedildi",
    "Access allowed" : "Erişim izni verildi",
    "Inherit permission" : "İzinler devralınsın",
    "Deny" : "Reddet",
    "Allow" : "İzin ver",
    "Group" : "Grup",
    "Team" : "Takım",
    "Could not save changes. Please try again." : "Değişiklikler kaydedilemedi. Lütfen yeniden deneyin.",
    "Advanced permissions for group folder" : "Grup klasörü için gelişmiş izinler",
    "Group, team, or user" : "Grup, takım veya kullanıcı",
    "Read" : "Okuma",
    "Write" : "Yazma",
    "Create" : "Oluşturma",
    "Delete" : "Silme",
    "Share" : "Paylaşma",
    "You" : "Siz",
    "Remove access rule" : "Erişim kuralını sil",
    "Inherited permissions" : "Devralınan izinler",
    "Inherited permission cannot be removed" : "Devralınan izin kaldırılamaz",
    "Add new rule" : "Yeni kural ekle",
    "Select a user or team" : "Bir kullanıcı ya da takım seçin",
    "Failed to update encryption setting" : "Şifreleme ayarı güncellenemedi",
    "Group folders encryption" : "Grup klasörleri şifrelemesi",
    "Encrypt group folders using server-side encryption. Remember that the data cannot be accessed if the encryption key is lost." : "Sunucu tarafı şifrelemeyi kullanarak grup klasörlerini şifreler. Şifreleme anahtarı kaybolursa verilere erişilemeyeceğini unutmayın.",
    "Enable group folders encryption" : "Grup klasörleri şifrelemesini aç",
    "Disabling group folders encryption is only possible using OCC, please refer to the documentation." : "Grup klasörleri şifrelemesi yalnızca OCC kullanılarak kapatılabilir. Lütfen belgelere bakın.",
    "Group folders encryption cannot be enabled on the server because server-side encryption is disabled." : "Sunucu tarafı şifreleme kapatıldığı için grup klasörleri şifrelemesi sunucuda açılamadı.",
    "Open group folder" : "Grup klasörünü aç",
    "List of group folders." : "Grup klasörleri listesi.",
    "No group folders yet" : "Henüz bir grup klasörü yok",
    "Group folders will show up here" : "Grup klasörleri burada görüntülenir",
    "Add group" : "Grup ekle",
    "Default" : "Varsayılan",
    "Unlimited" : "Sınırsız",
    "Folder could not be created" : "Klasör oluşturulamadı",
    "Are you sure you want to delete \"{folderName}\" and all files inside? This operation cannot be undone" : "\"{folderName}\" klasörünü içindeki tüm dosyalarla birlikte silmek istediğinize emin misiniz? Bu işlem geri alınamaz",
    "Delete \"{folderName}\"?" : "\"{folderName}\" silinsin mi?",
    "Folder could not be renamed" : "Klasör yeniden adlandırılamadı",
    "Group folder admin delegation" : "Grup klasörü yönetici ataması",
    "Nextcloud allows you to delegate the administration of group folders to non-admin users." : "Nextcloud, Grup klasörlerinin yönetimini yönetici olmayan kullanıcılara devretmenize izin verir.",
    "Specify below the groups that will be allowed to manage group folders and use its API/REST." : "Grup klasörlerini yönetmesine ve API/REST kullanmasına izin verilecek grupları aşağıda belirtin.",
    "They will have access to all group folders." : "Tüm grup klasörlerine erişebilirler.",
    "Specify below the groups that will be allowed to manage group folders and use its API/REST only." : "Grup klasörlerini yönetmesine ve yalnızca API/REST kullanmasına izin verilecek grupları aşağıda belirtin.",
    "They will only have access to group folders for which they have advanced permissions." : "Yalnızca gelişmiş izinlerine sahip oldukları grup klasörlerine erişebilirler.",
    "Group or team" : "Grup ya da takım",
    "Sort by number of groups or teams that have access to this folder" : "Bu klasöre erişimi olan grup veya takım sayısına göre sırala",
    "Sort by number of groups that have access to this folder" : "Bu klasöre erişimi olan grup sayısına göre sırala",
    "Team folder" : "Takım klasörü",
    "Folder name" : "Klasör adı",
    "This team folder belongs to a team and cannot be renamed" : "Bu takım klasörü bir takıma aittir ve yeniden adlandırılamaz",
    "Rename \"{mountPoint}\"" : "\"{mountPoint}\" takma noktasını yeniden adlandır",
    "Advanced permissions are managed by the team" : "Gelişmiş izinler takım tarafından yönetilir",
    "Enabled" : "Kullanılıyor",
    "Disabled" : "Kullanılmıyor",
    "Advanced permissions allows setting permissions on a per-file basis but comes with a performance overhead" : "Gelişmiş izinler, izinlerin dosya temelinde ayarlanmasını sağlar. Ancak başarım yükünü artırır",
    "Advanced permissions for \"{mountPoint}\"" : "\"{mountPoint}\" için gelişmiş izinler",
    "This folder belongs to a team and cannot be deleted directly; unlink it from its team first" : "Bu klasör bir takıma aittir ve doğrudan silinemez. Önce takım ile bağlantısını kaldırın",
    "No team folders yet" : "Henüz bir takım klasörü yok",
    "No group folders or team folders yet" : "Henüz bir grup klasörü veya takım klasörü yok",
    "Group folder name" : "Grup klasörü adı",
    "Create group folder" : "Grup klasörü oluştur",
    "Do not grant any advanced permissions by default" : "Varsayılan olarak herhangi bir gelişmiş izin verilmesin",
    "All" : "Tümü",
    "Quota" : "Kota",
    "Advanced Permissions" : "Gelişmiş izinler",
    "Actions" : "İşlemler",
    "Pagination of group folders and team folders" : "Grup klasörlerinin ve takım klasörlerinin sayfalaması",
    "Previous" : "Önceki",
    "Next" : "Sonraki",
    "Page:" : "Sayfa:",
    "User" : "Kullanıcı",
    "Unknown" : "Bilinmiyor",
    "Users/groups that can manage" : "Yönetilebilecek kullanıcılar ve gruplar",
    "None" : "Yok",
    "Unassign" : "Atamayı kaldır",
    "Assign groups" : "Gruplar ata",
    "Edit assigned groups: {groups}" : "Atanmış grupları düzenle: {groups}",
    "No other groups or teams available" : "Kullanılabilecek başka bir grup ya da takım yok",
    "No other groups available" : "Kullanılabilecek başka bir grup yok",
    "{displayName} (team)" : "{displayName} (takım)",
    "Add group or team" : "Grup ya da takım ekle",
    "{size} used" : "{size} kullanılmış",
    "Other …" : "Diğer…",
    "sorted descending" : "azalan sıralandı",
    "sorted ascending" : "artan sıralandı",
    "Admin configured folders shared with everyone in a team" : "Yönetici tarafından yapılandırılmış, takımdaki herkes ile paylaşılan klasörler",
    "Admin configured folders shared with everyone in a team.\n\nFolders can be configured from *Team folders* in the admin settings.\n\nAfter a folder is created, the admin can give access to the folder to one or more teams, control their write/sharing permissions and assign a quota for the folder.\nAs of Hub 10/Nextcloud 31, the admin needs to be a part of the team to be able to assign it a Team folder." : "Yönetici tarafından yapılandırılmış, takımdaki herkes ile paylaşılan klasörler.\n\nKlasörler, Yönetici ayarları *Takım klasörleri* bölümünden yapılandırılabilir.\n\nBir klasör oluşturulduktan sonra, yöneticiler klasöre bir ya da bir kaç takım için erişim, yazma ve paylaşım izinleri ile klasör boyutu kotasını ayarlayabilir. \nHub 10/Nextcloud 31 sürümünden başlayarak, yöneticinin bir takım klasörü atayabilmesi için takımın üyesi olması gereklidir.",
    "Advanced permissions for Team folder" : "Takım klasörü için gelişmiş izinler",
    "Select a user or group" : "Bir kullanıcı ya da grup seçin",
    "Open Team folder" : "Takım klasörünü aç",
    "Permission denied. User does not have sufficient permissions." : "İzin verilmedi. Kullanıcının yeterli izinleri yok.",
    "Unexpected status from server" : "Sunucudan beklenmeyen bir durum bildirimi alındı",
    "List of Team folders." : "Takım klasörleri listesi.",
    "No Team folders yet" : "Henüz bir takım klasörü yok.",
    "Team folders will show up here" : "Takım klasörleri burada görüntülenir",
    "Team folder admin delegation" : "Takım klasörü yönetici yetkisi devri",
    "Nextcloud allows you to delegate the administration of Team folders to non-admin users." : "Nextcloud, Takım klasörleri yönetim yetkisini yönetici olmayan kullanıcılara devretmenize izin verir.",
    "Specify below the groups that will be allowed to manage Team folders and use its API/REST." : "Takım klasörlerini yönetmesine ve API/REST kullanmasına izin verilecek grupları aşağıda belirtin.",
    "They will have access to all Team folders." : "Tüm takım klasörlerine erişebilirler.",
    "Specify below the groups that will be allowed to manage Team folders and use its API/REST only." : "Takım klasörlerini yönetmesine ve yalnızca API/REST kullanmasına izin verilecek grupları aşağıda belirtin.",
    "They will only have access to Team folders for which they have advanced permissions." : "Yalnızca gelişmiş izinlerine sahip oldukları takım klasörlerine erişebilirler.",
    "Advanced permissions are only supported with Nextcloud 16 and up" : "Gelişmiş izinler yalnızca Nextcloud 16 ve üzerindeki sürümlerde desteklenir",
    "Pagination of team folders" : "Takım klasörleri sayfalaması",
    "Team Folders" : "Takım klasörleri",
    "Shared folders managed by admins, accessible to designated teams with configurable permissions and quotas" : "Yöneticiler tarafından yönetilen, yapılandırılabilir izinleri ve kotaları olan belirli takımlar tarafından erişilebilen paylaşılan klasörler",
    "Team Folders (formerly \"Group Folders\") allows administrators to create and manage shared folders that are accessible\n\t\t\tto selected teams within Nextcloud.\n\n\t\t\tAdmins can configure folders from the Team Folders section in the admin settings, where they can grant access to one\n\t\t\tor more teams, set custom permissions (such as read, write, and sharing rights), and assign storage quotas to each\n\t\t\tfolder.\n\n\t\t\tAs of Hub 10/Nextcloud 31, admins must be members of a team to assign it a Team Folder. The app supports advanced\n\t\t\tfeatures such as quota management, granular access control, and integration with Nextcloud’s trash and versioning\n\t\t\tsystems." : "Takım klasörleri (eski adıyla \"Grup klasörleri\"), yöneticilerin Nextcloud üzerinde seçilmiş takımların erişebileceği \n\t\t\tpaylaşılan klasörler oluşturmasını ve yönetmesini sağlar.\n\n\t\t\tYöneticiler, yönetici ayarlarındaki Takım klasörleri bölümünden bir veya daha fazla takıma erişim izni verebilir, \n\t\t\tözel izinler (okuma, yazma ve paylaşma izinleri gibi) ayarlayabilir ve her klasöre depolama kotaları \n\t\t\tatayabilir.\n\n\t\t\tHub 10/Nextcloud 31 sürümüyle, yöneticiler bir Takım klasörü atamak için bir takımın üyesi olmalıdır. Uygulama, \n\t\t\tkota yönetimi, ayrıntılı erişim denetimi ve Nextcloud çöp ve sürüm sistemleri ile bütünleştirme gibi gelişmiş özellikleri \n\t\t\tdestekler.",
    "Team Folders (formerly \"Group Folders\") allows administrators to create and manage shared\nfolders for selected teams within Nextcloud.\n\nAdmins can grant one or more teams access to a folder, configure permissions (such as read,\nwrite, and sharing rights), and assign storage quotas from the Team Folders section (under\nadmin settings). The app also supports advanced permissions and integration with Nextcloud’s\ntrash and versioning systems.\n\nAs of Hub 10 / Nextcloud 31, admins must be members of a team to assign that team to a Team\nFolder." : "Takım Klasörleri (eski adıyla \"Grup Klasörleri\"), yöneticilerin Nextcloud üzerinde seçilmiş \ntakımlar için paylaşılan klasörler oluşturmasını ve yönetmesini sağlar.\n\nYöneticiler, bir veya birkaç takıma bir klasöre erişim izni verebilir, izinleri (okuma, \nyazma ve paylaşma gibi) yapılandırabilir ve Takım Klasörleri bölümünden (yönetici\nayarları altından) depolama alanı kotaları atayabilir. Uygulama ayrıca gelişmiş izinleri \ndestekler ve Nextcloud içindeki çöp ve sürüm sistemleri ile bütünleşik çalışır.\n\nHub 10 / Nextcloud 31 sürümüyle, yöneticiler bir takımı bir Takım Klasörüne atamak için \no takımın üyesi olmalıdır."
},
"nplurals=2; plural=(n > 1);");
