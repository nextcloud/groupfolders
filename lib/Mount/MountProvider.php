...
use OCA\GroupFolders\Folder\FolderWithPermissions;
use OCP\Constants;
...
class MountProvider implements IMountProvider {
	...
	/**
	 * @return list<FolderWithPermissions>
	 */
	...
	public function getMountsForUser(IUser $user, IStorageFactory $loader): array {
		...
		$mountPoints = array_map(fn (FolderWithPermissions $folder): string => 'files/' . $folder->mountPoint, $folders);
		$conflicts = $this->findConflictsForUser($user, $mountPoints);

		/** @var array<FolderWithPermissions> $foldersWithAcl */
		$foldersWithAcl = array_filter($folders, fn (FolderWithPermissions $folder): bool => $folder->acl);
		$rootFileIds = array_map(fn (FolderWithPermissions $folder): int => $folder->rootId, $foldersWithAcl);
		$aclManager = $this->aclManagerFactory->getACLManager($user);
		$rootRules = $aclManager->getRulesByFileIds($rootFileIds);

		return array_filter(array_map(function (FolderWithPermissions $folder) use ($user, $loader, $conflicts, $aclManager, $rootRules): ?IMountPoint {
