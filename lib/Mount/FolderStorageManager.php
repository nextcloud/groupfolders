...
class FolderStorageManager {
	...
	private function getObjectStorageBucket(int $folderId, array $objectStoreConfig): string {
		...
		if (!$bucket) {
			...
			$bucket = $objectStoreConfig['arguments']['bucket'] . $this->calculateBucketNum((string)$folderId, $objectStoreConfig);
	...
	private function calculateBucketNum(string $key, array $objectStoreConfig): string {
		...
		$minBucket = isset($objectStoreConfig['arguments']['min_bucket'])
			? (int)$objectStoreConfig['arguments']['min_bucket']
			: 0;
