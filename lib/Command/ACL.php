			...
			default:
				$items = array_map(function (array $rulesForPath, string $path) use ($jailPathLength): array {
					...
					$mappings = array_map(function (Rule $rule): string {
						...
						$mappingDisplayName = $rule->getUserMapping()->getDisplayName();
						return $rule->getUserMapping()->getType() . ': '
							. ($mappingId === $mappingDisplayName ? $mappingId : $mappingDisplayName . ' (' . $mappingId . ')');
					}, $rulesForPath);
