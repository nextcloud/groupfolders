...
class ListCommand extends Base {
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		usort($folders, fn (Folder $a, Folder $b): int => $a->id - $b->id);
		...
		if ($outputType === self::OUTPUT_FORMAT_JSON || $outputType === self::OUTPUT_FORMAT_JSON_PRETTY) {
			$formatted = array_map(fn (FolderWithMappings $folder): array => $folder->toArray(), $folders);
			...
			$table->setRows(array_map(function (FolderWithMappings $folder) use ($groupNames): array {
