...
class FolderManager {
	...
	private function rowToFolder(array $row): Folder {
		return new Folder(
			(int)$row['folder_id'],
	...
	public function getFoldersForGroup(string $groupId): array {
		...
		return array_values(array_map(function (array $row): FolderWithPermissions {
			$folder = $this->rowToFolder($row);
			return FolderWithPermissions::fromFolder(
				$folder,
				Cache::cacheEntryFromData($row, $this->mimeTypeLoader),
	...
	public function getFoldersForUser(IUser $user): array {
		$groups = $this->groupManager->getUserGroupIds($user);
		/** @var list<FolderWithPermissions> $folders */
		$folders = array_merge(
			$this->getFoldersForGroups($groups),
			$this->getFoldersFromCircleMemberships($user),
		);

		/** @var array<int, FolderWithPermissions> $mergedFolders */
		$mergedFolders = [];
