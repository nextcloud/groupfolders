...
class TrashBackend implements ITrashBackend {
        ...
        public function moveToTrash(IStorage $storage, string $internalPath): bool {
                if ($storage->instanceOfStorage(GroupFolderStorage::class) && $storage->isDeletable($internalPath)) {
                        /** @var GroupFolderStorage|Jail $storage */
                                $mountPoint = trim($storage->getMountPoint(), '/');
                                [,, $mountPoint] = explode('/', $mountPoint);
                                $fakePath = '';
                                $folderId = $storage->getFolderId();
                                $fakeTrash = new FakeTrashHierarchy($storage, $internalPath, $mountPoint);
                                $trashFolder = $this->getTrashFolder($folderId);
                                $rootFakeTrashFolder = array_filter($this->trashManager->listTrashForFolders([$folderId]), function($item) use ($folderId) {
                                ...
                                if($fakeTrash->rootDirExists($trashFolder, $rootFakeTrashFolder[0]['deleted_time'])) {
                                        // the root directory exists
                                        $rootDirTime = $rootFakeTrashFolder[0]['deleted_time'];
                                        if(strpos($internalPath, '/')) {
                                                //check if the structure exists in trash
                                                //and otherwise recreate it
                                                $dir = pathinfo($mountPoint . '/' . $internalPath, PATHINFO_DIRNAME);
                                                $neededFolders = $fakeTrash->getNeededFolders($trashFolder, $dir, $rootDirTime);
                                                $fakePath = substr($dir, 0, strpos($dir, '/')) . '.d' . $rootDirTime . '/';

                                                if($neededFolders !== '') {
                                                        $created = $fakeTrash->create($trashFolder, $neededFolders);
                                                        [$internalPath, $trashName] = $fakeTrash->getPathAndName($created);
                                                } else {
                                                        $fakePath .= pathinfo($internalPath, PATHINFO_DIRNAME) . '/';
                                                        $trashName = $name;
                                                }
                                        } else {
                                                $fakePath = $mountPoint . '.d' . $rootDirTime . '/';
                                                $trashName = $name;
                                        }
                                } else {
                                        $created = $fakeTrash->create($trashFolder);
                                        [$internalPath, $trashName] = $fakeTrash->getPathAndName($created);
                                ...
                                $targetInternalPath = $trashFolder->getInternalPath() . '/' . $fakePath . $trashName;
        ...
        private function getTrashForFolders(IUser $user, array $folders): array {
                ...
                $items = [];
                $view = new View('/' . $user->getUID() . '/files_trashbin/files/');
