...
class TrashBackend implements ITrashBackend {
	...
	private function setupTrashFolder(FolderWithPermissions $folder, ?IUser $user = null): Folder {
		$folderId = $folder->id;
		if ($user) {
	...
	/**
	 * @param list<FolderWithPermissions> $folders
	 * @return list<ITrashItem>
	 */
	private function getTrashForFolders(IUser $user, array $folders): array {
		$folderIds = array_map(fn (FolderWithPermissions $folder): int => $folder->id, $folders);
	...
	public function cleanTrashFolder(FolderWithPermissions $folder): void {
	...
	public function expire(Expiration $expiration): array {
		...
		$folders = array_map(function (FolderWithMappingsAndCache $folder): FolderWithPermissions {
			return FolderWithPermissions::fromFolder($folder, $folder->rootCacheEntry, Constants::PERMISSION_ALL);
		}, $folders);
		foreach ($folders as $folder) {
			$folderId = $folder->id;
	...
	/**
	 * @param array<int, FolderWithPermissions> $existingFolders
